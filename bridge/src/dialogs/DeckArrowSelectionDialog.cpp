...
DeckArrowSelectionDialog::DeckArrowSelectionDialog(bool isDeck) :
	...
	m_adjustment = gtk_adjustment_new(
			m_isDeck ? getCardWidth() : getArrowSize(), lower, upper,
			ADJUSTMENT_STEP_INCREMENT, 0, ADJUSTMENT_PAGE_SIZE);
	...
	for (i = 0; i < 2; i++) {
		...
		border[i] = gtk_frame_new(
				getString(i == 0 ? STRING_RASTER : STRING_VECTOR));
	...
	for (i = 0; i < (m_isDeck ? N_TOTAL_DECKS : N_TOTAL_ARROWS); i++) {
		...
		if (n == 0) {
			...
			radio = m_radio[i] = gtk_radio_button_new_with_label_from_widget(rb,
					p);
		}
...
bool DeckArrowSelectionDialog::click(int index) {
	if (index == 0) {
		if (m_isDeck) {
			gconfig->setDeckParameters(m_deckNumber,
					gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(m_check)),
					getObjectSize());
...
void DeckArrowSelectionDialog::draw(cairo_t *cr) {
	...
	if (sc) {
		...
		if (cs != m_lastDrawnSize) {
			m_lastDrawnSize = cs;
			getProblemSelector().drawSvg(cs, getNumberIndex(), m_isDeck,
					getValue());
		}
	} else {
...
void DeckArrowSelectionDialog::setHideShowWidgets() {
	...
	GtkWidget *a[] = { m_area, m_scale, m_loading, m_label };
