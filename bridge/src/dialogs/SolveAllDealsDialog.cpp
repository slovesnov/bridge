...
SolveAllDealsDialog::SolveAllDealsDialog() :
	...
	for (auto &a : m_button) {
		...
		g_signal_connect(a, "clicked", G_CALLBACK(button_clicked), gpointer(0));
		i++;
	}
	...
	for (i = 0; i < SIZEI(PLAYER); i++) {
		if (isPreferans() && PLAYER[i] == getAbsent()) {
			...
			if (isBridge()) {
				...
				t = PLAYER[i] == p.m_player ?
						PLAYERBOX_NAME_TYPE_UNDERLINED :
						PLAYERBOX_NAME_TYPE_EMPTY_BOX;
			}
	...
	for (i = 0; i < resultSize(); i++) {
		gtk_grid_attach(GTK_GRID(g1), gtk_label_new(std::to_string(i).c_str()),
				k + 1, i + 1, 1, 1);
		for (j = 0; j < 2; j++) {
			gtk_grid_attach(GTK_GRID(g1), m_label[i][j], k + j + 2, i + 1, 1,
					1);
		}
	}
	...
	w = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
	gtk_container_add(GTK_CONTAINER(w),
			m_button[EXPORT_ALL_DEALS_WITH_RESULTS_TO_CSV_BUTTON]);
	s = "csv ";
	...
	if (isBridge()) {
		...
		gtk_container_add(GTK_CONTAINER(w),
				gtk_label_new(getString(STRING_PLAYERS_WITH_UNKNOWN_CARDS)));
		...
		gtk_combo_box_set_active(GTK_COMBO_BOX(m_combo[TAB1]),
	...
	gtk_grid_attach(GTK_GRID(g1), gtk_label_new(getString(STRING_THREAD)), j, 0,
			1, 1);
...
void SolveAllDealsDialog::updateData() {
	...
	gtk_label_set_text(GTK_LABEL(m_labelTotal),
			intToStringLocaled(m_total).c_str());
	gtk_label_set_text(GTK_LABEL(m_labelTotalTime),
			getTotalTimeLabelString().c_str());
...
void SolveAllDealsDialog::comboChanged(GtkWidget *w) {
	...
	if (i == TAB1) {				//only bridge
		stopExportThread();
		setBridgeSolveAllDealsAbsentNS(
				!gtk_combo_box_get_active(GTK_COMBO_BOX(m_combo[TAB1])));
		m_map.clear();
		m_mapLC.clear();
		...
		for (i = 0; i < 4; i++) {
			...
			if (isBridgeSolveAllDealsAbsentNS() == northOrSouth(PLAYER[i])) {
				...
				gtk_container_add(GTK_CONTAINER(w),
						label(getPlayerString(PLAYER[i])));
			}
...
GtkWidget* SolveAllDealsDialog::createTab2() {
	...
	if (bridge) {
		m_combo[BRIDGE_DOUBLE_REDOUBLE_COMBO] = createTextCombobox(
				STRING_UNDOUBLED, 3);
		m_combo[BRIDGE_VULNERABLE_COMBO] = createTextCombobox(STRING_NO,
				STRING_YES);
	} else {
		m_combo[PREFERANS_PLAYERS_COMBO] = createTextCombobox(3, 4);
		s = getString(STRING_WHIST_OPTIONS_COMBO);
		m_combo[PREFERANS_WHIST_OPTION_COMBO] = createTextCombobox(
				split(s, '#'));
	}
	...
	for (j = 0; j < TITLE_ROWS; j++) {
		if (j == 0) {
			...
			if (bridge) {
				...
				VStringID vi = {
						isMisere() ? STRING_MISERE_PLAYER : STRING_DECLARER };
				for (i = 0; i < 3; i++) {
	...
	for (i = 1; i < 3; i++) {
		...
		if (bridge || (!bridge && i == PREFERANS_PLAYERS_COMBO)) {
			if (bridge) {
				id = i == BRIDGE_DOUBLE_REDOUBLE_COMBO ?
						STRING_CONTRACT : STRING_VULNERABLE;
			} else {
				id = STRING_PLAYERS;
			}
...
void SolveAllDealsDialog::updateTab2() {
	...
	if (isBridge()) {
		const int trump = getTrump();
		const int doubleRedouble = getComboPosition(
				m_combo[BRIDGE_DOUBLE_REDOUBLE_COMBO]);
		const bool vulnerable = getComboPosition(
				m_combo[BRIDGE_VULNERABLE_COMBO]);
		...
		const int option = getComboPosition(
				m_combo[PREFERANS_WHIST_OPTION_COMBO]);
		PreferansScore p;
			...
			for (auto c : { 0, 6, 7, 8, 9, 10 }) {
				for (i = 0; i < WHIST_OPTION_SIZE; i++) {
					...
					if (wo == WHIST_OPTION_WHIST || c == tricks) {
						v[tricks][c][i] = p.getGameScore(players, c, tricks,
								wo);
					}
				}
			}
...
std::string SolveAllDealsDialog::getNSEWString(bool ns) {
	...
	return getLowercasedPlayerString(i) + " / "
			+ getLowercasedPlayerString(getBridgePartner(i));
}
...
//returns {label,check}
std::pair<GtkWidget*, GtkWidget*> SolveAllDealsDialog::addClickableLabelWithCheck(
		GtkWidget *w, std::string s) {
	auto l = label(s);
	//allow click on label
	auto q1 = gtk_event_box_new();
	gtk_container_add(GTK_CONTAINER(q1), l);
	g_signal_connect(q1, "button-press-event", G_CALLBACK(label_click), l);

	auto c = gtk_check_button_new();
	gtk_container_add(GTK_CONTAINER(w), c);
	g_signal_connect(c, "toggled", G_CALLBACK(check_changed), l);
