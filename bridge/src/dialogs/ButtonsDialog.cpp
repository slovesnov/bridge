...
static const STRING_ID TYPE[][MSIZE] = {
		{ STRING_INVALID },
		{ STRING_OK, STRING_INVALID },
		{ STRING_OK, STRING_CANCEL, STRING_INVALID },
		{ STRING_YES, STRING_NO, STRING_CANCEL, STRING_INVALID },
		{
				STRING_TO_ONE_FILE,
				STRING_TO_MANY_FILES,
				STRING_ADD,
				STRING_CLEAR,
				STRING_CANCEL,
				STRING_INVALID },
		{ STRING_OK, STRING_CANCEL, STRING_CLEAR, STRING_INVALID },
		{ STRING_OK, STRING_CANCEL, STRING_RESTORE_DEFAULT, STRING_INVALID } };

static const char* IMG[][MSIZE - 1] = { { }, { "ok" }, { "ok", "cancel" }, {
		"ok",
		"cancel",
		"cancel" }, { "ok", "split", "add", "clear", "cancel" }, {
		"ok",
		"cancel",
		"clear" }, { "ok", "cancel", "restore" } };
...
void ButtonsDialog::init(BUTTONS_DIALOG_TYPE type) {
	...
	for (i = 0, p = TYPE[type]; *p != STRING_INVALID; p++, i++) {
		...
		g_signal_connect(w, "clicked", G_CALLBACK(button_clicked), gpointer(this));
	}
