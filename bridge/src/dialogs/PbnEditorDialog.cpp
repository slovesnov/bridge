...
const char *CM[] = { PBN_DEALER_TAG, PBN_RESULT_TAG, PBN_ROOM_TAG,
		PBN_VULNERABLE_TAG };
...
PbnEditorDialog::PbnEditorDialog() :
		ButtonsDialog(MENU_PBN_EDITOR), ProblemVectorModified(
				getProblemSelector()) {
	int i;
	GtkWidget *w, *w1;
		...
		if ((j = INDEX_OF_NO_CASE(it->tag, CM)) != -1) {
			if (j == 1) {
				if (!getProblem().noTrumpOrContract()) {
					...
					gtk_grid_attach(GTK_GRID(m_grid), m_combo[j], k + 1, i, 1,
							1);
				}
			} else {
				if (j == 0) {
			...
			gtk_container_add(GTK_CONTAINER(w1),
					m_combo[PBN_EDITOR_COMBO_SCORE]);
			gtk_label_set_width_chars(
					GTK_LABEL(m_label[PBN_EDITOR_LABEL_SCORE]), 5);
			gtk_box_pack_start(GTK_BOX(w1), m_label[PBN_EDITOR_LABEL_SCORE],
					TRUE,
					TRUE, 2);
			...
			if (getProblem().noTrumpOrContract()) {
				...
				if (getProblem().m_trump == NT) {
					w = gtk_label_new(
							getProblem().getEnglishTrumpString().c_str()); //language independent
				} else {
					w = gtk_image_new_from_pixbuf(
							getSuitPixbuf(getProblem().m_trump,
									getFontHeight()));
				}
				...
				gtk_box_pack_start(GTK_BOX(w1),
						m_combo[PBN_EDITOR_COMBO_DOUBLE_REDOUBLE], TRUE, TRUE,
						2);
