...
const STRING_ID ALL_SUPPORTED[] = {
    STRING_FILE_FILTER_ALL_SUPPORTED, STRING_FILE_FILTER_BTS_PTS,
...
const STRING_ID STRING_FILTER_EXT_FROM[] = {
    ...
    STRING_FILE_FILTER_HTML, STRING_FILE_FILTER_LANGUAGE, STRING_IMAGE_FILES,
    STRING_CSV_FILES,
    STRING_FILE_FILTER_ALL_SUPPORTED, // should goes after all simple filters,
                                      // see
                                      // 'if(option==CHOOSER_OPTION_DEFAULT){'
                                      // in Widget::fileChooser()
    STRING_FILE_FILTER_ALL            // should goes last
};

const char *STRING_FILTER_EXT_TO[] = {
    "bts pts", "pbn", "txt dat", "htm html", "lng",
...
static void drag_and_drop_received(GtkWidget *, GdkDragContext *context, gint x,
                                   gint y, GtkSelectionData *data, guint ttype,
                                   guint time, Widget *widget) {
...
Widget::Widget(GtkWidget *widget) { setWidget(widget); }

Widget::~Widget() {}

DrawingArea &Widget::getArea() const { return gframe->m_area; }

LastTrick &Widget::getLastTrick() const { return *glasttrick; }

Toolbar &Widget::getToolbar() const { return gframe->m_toolbar; }

bool Widget::isModified() const { return getProblemSelector().isModified(); }

bool Widget::isEditEnable() const { return getToolbar().isEditEnable(); }

void Widget::enableEdit(bool enable, bool anyway /*=false*/) {
  gframe->enableEdit(enable, anyway);
}

bool Widget::isUndoEnable() const { return getArea().isUndoEnable(); }

bool Widget::isRedoEnable() const { return getArea().isRedoEnable(); }
...
void Widget::updateModified() { gframe->updateModified(); }

FileChooserResult Widget::fileChooser(MENU_ID menu, FILE_TYPE filetype,
                                      CHOOSER_OPTION option,
  ...
  /* if user click close button in dialog (in top right corner "X" where three
   * buttons) gtk_dialog_run returns GTK_RESPONSE_DELETE_EVENT so use for cancel
   * button same code if user make double click on file then gtk_dialog_run
   * returns code of first button so it doesn't matter which code use for other
  ...
  dialog = gtk_file_chooser_dialog_new(
      getString(menu), GTK_WINDOW(gframe->getWidget()),
      menu == MENU_OPEN ? GTK_FILE_CHOOSER_ACTION_OPEN
  ...
  if (option == CHOOSER_OPTION_DEFAULT) {
    ...
    assert(sid != STRING_FILE_FILTER_ALL_SUPPORTED &&
  ...
  for (auto vit : vid) {
     ...
     * if 'save' option all supported formats (*.bts *.pts *.pbn *.txt *.dat
     * *.htm *.html)
     */
  ...
  if (result.ok()) {
    if (menu == MENU_OPEN) {
      ...
      if (option == CHOOSER_OPTION_SAVE_AS_MANY ||
        ...
        if (defaultFiletype ==
            FILE_TYPE_ERROR) { // check filter on invalid file extension
...
FileChooserResult Widget::fileChooser(MENU_ID menu, const std::string &filepath,
  ...
  return fileChooser(menu, ft,
                     allowSplit ? CHOOSER_OPTION_SAVE_AS_MANY
...
void Widget::correctFileExtension(std::string &filepath, FILE_TYPE filetype) {
  /* if user set invalid extension or empty extension add it to make filename
   * valid For example "name.1.2" -> "name.1.2.html"
   */
...
void Widget::openUris(char **uris) { getProblemSelector().openUris(uris); }
...
void Widget::setFont(cairo_t *cr, int height) {
  cairo_select_font_face(
      cr, gconfig->getFontFamily(), cairo_font_slant_t(gconfig->getFontStyle()),
      gconfig->getFontWeight() >= PANGO_WEIGHT_BOLD ? CAIRO_FONT_WEIGHT_BOLD
...
GtkWidget *Widget::createTextCombobox(const VString &text) {
  ...
  // g_object_set (G_OBJECT (renderer),"font", family,"style", style,"size",
  // 20,NULL);//don'n remove leave for further needs
...
int Widget::getFontHeight() const { return gconfig->getFontHeight(); }
...
void Widget::menuClick(MENU_ID id) { gframe->menuClick(id); }

bool Widget::isTrumpChanged() const { return getToolbar().isTrumpChanged(); }
...
/*
 ...
 return getToolbarPixbuf(id, small,
 boolToButtonState(isToolbarButtonEnabled(id)));
 }


 GdkPixbuf* Widget::getToolbarPixbuf(TOOLBAR_BUTTON id, bool small, bool
 enabled) { return getToolbarPixbuf(id, small, boolToButtonState(enabled));
 }
 */
...
int Widget::getArrowSize() { return gconfig->m_arrowSize; }

void Widget::setArrowParameters(int arrow, int arrowSize /*=SKIP_ARROW_SIZE*/) {
  gconfig->setArrowParameters(arrow, arrowSize);
}

CPoint Widget::getCardSize() const { return gconfig->getCardSize(); }

int Widget::getCardWidth() { return gconfig->getCardWidth(); }

int Widget::getCardHeight() { return gconfig->getCardHeight(); }
...
int Widget::getMaxRunThreads() const { return gconfig->m_maxThreads; }
...
bool Widget::isScalableArrow() { return gconfig->isScalableArrow(); }

bool Widget::isScalableArrow(int arrow) {
  return gconfig->isScalableArrow(arrow);
}

bool Widget::isScalableDeck() { return gconfig->isScalableDeck(); }

bool Widget::isScalableDeck(int deck) { return gconfig->isScalableDeck(deck); }

GdkRGBA Widget::getTextColor() const { return gconfig->getFontColor(); }

GdkRGBA *Widget::getFontColorPointer() { return &gconfig->getFontColor(); }

PangoLayout *Widget::createPangoLayout(cairo_t *cr, TextWithAttributes text) {
  std::string o;
  PangoLayout *layout = pango_cairo_create_layout(cr);
  PangoFontDescription *desc =
      createPangoFontDescription(gconfig->m_font, text.getHeight());
...
CPoint Widget::getMaxSize() const { return gconfig->m_workareaRect.size(); }

void Widget::updateUndoRedoAll() { return gframe->updateUndoRedo(); }

void Widget::updateFindBestStateAll() { return gframe->updateFindBestState(); }

void Widget::updateThinkAll() { return gframe->updateThink(); }

int Widget::recentSize() { return gconfig->recentSize(); }

Menu &Widget::getMenu() { return *gmenu; }
...
BUTTON_STATE Widget::getFindBestState(bool checkTrump) const {
  return think() ? BUTTON_STATE_FIND_BEST_STOP
...
bool Widget::isEmptyDeal() const { return getDealState() == DEAL_STATE_EMPTY; }
...
CARD_INDEX Widget::getDeclarer() const { return getProblem().getDeclarer(); }

int Widget::languages() { return gconfig->m_language.size(); }

bool Widget::isLanguage(MENU_ID id) {
  return id >= MENU_LANGUAGE_FIRST && id < MENU_LANGUAGE_FIRST + languages();
}

int Widget::getActiveCardShift() { return gconfig->getActiveCardShift(); }

CARD_INDEX &Widget::getPlayer() { return getProblem().m_player; }

CARD_INDEX Widget::getPlayer() const { return getProblem().m_player; }

VString &Widget::recent() { return gconfig->m_recent; }
...
std::string &Widget::csvSeparator() { return gconfig->m_csvSeparator; }

VSolveAll &Widget::solveAll() { return gdraw->getSolveAll(); }

SolveAll &Widget::solveAll(int i) { return solveAll()[i]; }

VCardIndex Widget::getVariablePlayers() {
  if (isBridge()) {
    auto a =
        isBridgeSolveAllDealsAbsentNS() ? CARD_INDEX_NORTH : CARD_INDEX_EAST;
    return {a, getBridgePartner(a)};
  } else {
    auto &pr = getProblem();
...
std::string Widget::getLanguageDependedUrl(std::string const &s) {
  return gconfig->getLanguageString() == "russian" ? s + ",ru" : s;
}

void Widget::openHomepage() { openURL(getLanguageDependedUrl(HOMEPAGE)); }
