...
static gboolean draw_tooltip_background(GtkWidget *widget, cairo_t *cr,
		gpointer) {
...
Toolbar::Toolbar() :
	...
	g_signal_connect(G_OBJECT (m_tooltip), "draw",
			G_CALLBACK (draw_tooltip_background), 0);
...
void Toolbar::updateButton(TOOLBAR_BUTTON id) {
	gtk_widget_set_sensitive(GTK_WIDGET(m_button[id]),
			isToolbarButtonEnabled(id));
...
//	GdkPixbuf *m_stopPixbuf[2]; //[small] enable always true
	if (id == TOOLBAR_BUTTON_FIND_BEST
			&& state == BUTTON_STATE_FIND_BEST_STOP) {
